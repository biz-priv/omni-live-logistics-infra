resource "aws_vpc" "sightline_vpc" {
  cidr_block = var.vpc_cidr # Change the CIDR block as needed
  enable_dns_support = true
  enable_dns_hostnames = true
  tags = {
    Name = "sightline_vpc" # Set the VPC name as "sightline_vpc"
  }
}
...
resource "aws_subnet" "public_subnet_2" {
  ...
  availability_zone = var.az2 # Specify the desired availability zone
  map_public_ip_on_launch = true # Make the subnet public
  tags = {
    Name = "public_subnet" # Set the subnet name as "public_subnet"
  }
...
output "vpc_id" {
  value = aws_vpc.sightline_vpc.id
}
