  ...
  dashboard_body = jsonencode({
    "widgets": [
        ...
        },
        {
            ...
            "height": 6,
            "properties": {
                "query": "SOURCE 'sightline' | fields @logStream, @message | filter @logStream = 'sightline2' | parse @message \"Main: sightline, Method: *, Status: *\" as method, status | filter ispresent(method) and ispresent(status) | stats count() as requests by method, status ",                "region": "us-east-1",
                "stacked": false,
                "view": "pie"
            }
        }
